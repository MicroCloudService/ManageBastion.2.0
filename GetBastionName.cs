...
{
    public static class GetBastionName
    {
        [FunctionName("GetBastionName")]
            ...
            ILogger log)
        {
            ...
            string varbastionName = config["bastionName"];
            string varBastionId = config["bastionId"];
            
            string varpublicIp = config["publicIp"];
            string varvNet= config["vNet"];
            string varlocation= config["location"]; 
            string varsku= config["sku"];

            log.LogInformation("C# HTTP trigger function processed a request.");

            var myObj = new { bastionName = varbastionName ,
                bastionId = varBastionId,
                publicIP = varpublicIp,
                vNet= varvNet ,
